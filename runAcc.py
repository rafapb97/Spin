...
test_data = np.load("x_test.npz")['arr_0'][0:num_test]
test_labels = np.load("y_test.npz")['arr_0'][0:num_test]
...
for j in test_data:
    ...
    spikes_brains = list()
    for brain in network[:-2]:
        spikes_brains.append(brain.get_data("spikes").segments[0].spiketrains)



    #generate some plots
    for i, spikes_brain in zip(range(len(network[:-2])), spikes_brains):
