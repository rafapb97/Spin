...
spikes_brains = list()
segments=[]
spikes_brains.append(network[-1].get_data("spikes").segments[0].spiketrains)
segments.append(network[-1].get_data("spikes").segments[0])
...
for (nrn, spike_train) in enumerate(spikes_brains[0]):
	ax_spikes.plot(spike_train, np.ones_like(spike_train) * nrn, "|", c='r', ms=5)

mn_spiketimes = [time for spikes in spikes_brains[0] for time in spikes]
